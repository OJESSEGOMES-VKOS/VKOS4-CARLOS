...
6. Logo: se o comprador tiver um `identidade/logo/logo.png`, **copie o arquivo pra `img/logo.png`
   dentro da pasta da peça** e aponte o `src` pra `img/logo.png`. O renderizador só enxerga
   arquivo de dentro da pasta do carrossel, então logo apontado direto pra `identidade/logo/`
   não aparece na imagem, e some sem dar erro. Se ele não tiver logo, remova as tags
   `<img class="logo">` / `<img class="cta-mark">` (o layout aguenta sem).
7. Modelo Produto sem foto do produto: apague a linha do `<img class="product">` e tire o
   comentário do `<div class="product-fallback">`, que está logo abaixo dela no modelo.
...
Garanta que o Playwright está instalado (se `node_modules` não existe, rode
`npm ci --ignore-scripts`; se o navegador faltar, `npm run setup`). Então confira a peça e
renderize:

```
npm run modelo:check -- conteudo/<AAAA-MM-DD>-<tema-curto>/carrossel.html
node templates/carrossel/render.js conteudo/<AAAA-MM-DD>-<tema-curto>
```

O verificador pega em segundos o erro que some sem avisar: imagem, foto ou logo apontando pra fora
da pasta da peça. Saiu alguma linha, conserte antes de renderizar.
