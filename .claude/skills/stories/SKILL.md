...
6. Confira a peça e renderize:
   `npm run modelo:check -- conteudo/<AAAA-MM-DD>-<tema>/stories.html` e depois
   `node templates/stories/render.js conteudo/<AAAA-MM-DD>-<tema>`. As imagens saem em
   `.../instagram-stories/story-01.png`, etc. (Precisa do Playwright, igual ao carrossel.) O
   verificador pega imagem ou logo apontando pra fora da pasta da peça, que some do PNG sem erro.
   Se o comprador tem logo, copie `identidade/logo/logo.png` pra `img/logo.png` dentro da pasta da
   peça e aponte pra lá.
