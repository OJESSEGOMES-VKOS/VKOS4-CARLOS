...
Esta skill verifica design e parte da implementação visual; ela não substitui auditoria de
segurança. Se a peça tiver formulário, script, backend, auth, banco, pagamento, upload, IA ou dado
pessoal, incluir `/seguranca` como gate separado antes de publicar.

Leia, nesta ordem:

1. `templates/<formato>/principios-visuais.md` do formato da peça (pra interface de projeto,
   use o de site, que é o mais próximo).
   Se a peça é **site, landing ou interface** (qualquer coisa com `.html` que abre no
   navegador), leia também `templates/site/principios-tecnicos.md`. Nessas peças a auditoria
   tem seis áreas, não cinco: o que a página faz conta tanto quanto a cara dela.
...
**Sexta área, só pra site, landing e interface: o que a página faz.** Régua em
`templates/site/principios-tecnicos.md`, seção 6. Confira nesta ordem, que é a de maior
prejuízo pro dono:

| O que olhar | O defeito que você procura |
|---|---|
| O contato chega | `<form>` sem destino, botão de WhatsApp sem `encodeURIComponent`, `href="#"` esquecido, número errado |
| Protege | Chave ou senha dentro do arquivo, `target="_blank"` sem `rel="noopener"`, `innerHTML` com texto de fora, sem linha de consentimento |
| Todo mundo usa | Some o foco ao navegar por Tab, campo sem `<label>`, imagem sem `alt`, falta `lang="pt-BR"`, falta `prefers-reduced-motion`, alvo de toque menor que 44px |
| É achada | Sem `title` com serviço e cidade, sem `og:image`, sem bloco `LocalBusiness`, ou pior: `LocalBusiness` com dado inventado |
| É rápida | Imagem acima de 300 KB, imagem sem `width`/`height`, imagem do topo com `lazy`, fonte sem `display=swap` |

Formulário que não envia e `LocalBusiness` com dado inventado são **falhas graves**, no mesmo
nível de uma proibição visual. O primeiro faz o dono perder cliente sem saber; o segundo manda
cliente pro endereço errado.
