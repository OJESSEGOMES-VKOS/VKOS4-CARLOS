...
objeções, cidade e CTA. Leia também `marca/conversao.md`: a ordem é ICP → nível de consciência
...
Antes do primeiro arquivo, leia `security/BASELINE.md`. Se houver formulário ou terceiro, aplique
`security/LGPD-CHECKLIST.md`; se o pedido evoluir para login, backend, banco, pagamento, upload ou
IA, pare o fluxo estático e use `/projeto`. Antes de publicar, rode `/seguranca`.
...
1. **Leia antes de qualquer código:** `templates/site/principios-visuais.md` INTEIRO (a lei da
   cara: cor, tipografia, layout, motion, proibições, teste final e o contrato do Studio na
   seção 6) e `templates/site/principios-tecnicos.md` INTEIRO (a lei do que a página faz:
   formulário que envia de verdade, segurança, acessibilidade, o selo do Google e velocidade).
   Os dois são obrigatórios: um site lindo que não captura contato ou que ninguém acha no
   Google é meio site. Leia também `templates/design/cartela.md` (a cartela de direções) e
...
6. **Siga o contrato do Studio (seção 6 do princípios) na íntegra.** Seções semânticas filhas
   ...
   pela metade.
7. **O contato tem que chegar (seção 1 do técnico).** Todo botão de ação e todo formulário
   leva a algum lugar real. O padrão do negócio local é o WhatsApp montado com
   `encodeURIComponent`, usando o número do Cérebro. Formulário sempre com `<label>` ligado, o
   campo isca contra robô e a linha de consentimento ao lado do botão. Formulário que não envia
   nada é o defeito mais caro do sistema, porque o dono só descobre meses depois, sem cliente.
8. **O site precisa ser achado (seções 2 e 4 do técnico).** `<html lang="pt-BR">`, `title` com
   serviço e cidade, `description` que convida, `og:image` (é o que faz o link chegar bonito
   quando ele manda no WhatsApp) e o bloco `LocalBusiness` preenchido **só com dado real do
   Cérebro**. Nunca invente endereço, telefone ou horário: dado errado aqui manda cliente pro
   lugar errado. Mais `robots.txt`, `sitemap.xml` e favicon na raiz.
9. **Rode os dois testes finais antes de entregar:** o visual (seção 5 do princípios visuais),
   com a pergunta "parece IA?", o teste de reflexo nas duas ordens, mobile de 390px sem estouro
   e contraste conferido; e o técnico (seção 6 do princípios técnicos), com funciona, protege,
   todo mundo usa, é achada e é rápida. Item que falhou volta pra bancada. Só entregue o que
   passar nos dois.
10. **Entregue simples:** diga onde o site ficou, que dá pra ver abrindo o `index.html` com dois
   cliques, e aponte o caminho de publicação do "Como colocar no ar". Fale do que muda pro
   negócio dele (aparece no Google, o contato chega, abre rápido no celular), nunca em termo
   técnico: a tabela da seção 7 do técnico traduz. Faltou algo que só o comprador tem (uma foto,
   um preço)? A página sai completa com o que existe e a pendência vai listada no seu recado
   final, nunca como buraco dentro da página.
...
5. **Bonito é metade.** A outra metade é o contato chegar, o Google achar, todo mundo conseguir
   usar e a página abrir rápido no celular da rua. Site que só é bonito não vende.
